/**
 * @Author: haru
 * @Date:   2017-05-27T07:03:20+09:00
 * @Last modified by:   haru
 * @Last modified time: 2017-05-27T07:38:35+09:00
 */
    ...
    $.fn.formToWizard = function( options, cmdParam1 ) {
        ...
        if( typeof options !== 'string' ) {
            ...
            steps.each(function(i) {
                ...
                if( options.showProgress && typeof(options.progress) !== "function") {
                    if( options.showStepNo )
                        $("#steps").append("<li id='stepDesc" + i + "'>ステップ " + (i + 1) + "<span>" + $(this).find("legend").html() + "</span></li>");
                    else
        ...
        function createPrevButton(i) {
            ...
            $('#' + stepName + 'commands').append(
                '<' + options.buttonTag + ' href="#" id="' + stepName + '次へ" class="' + options.prevBtnClass + '">' +
                options.prevBtnName +
                '</' + options.buttonTag + '>'
            );
        ...
        function createNextButton(i) {
            var stepName = 'step' + i;
            $('#' + stepName + 'commands').append(
                '<' + options.buttonTag + ' href="#" id="' + stepName + '次へ" class="' + options.nextBtnClass + '">' +
                options.nextBtnName +
                '</' + options.buttonTag + '>');
